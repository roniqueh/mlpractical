...
class SELUInit(object):
	...
	def __init__(self, rng=None):
		"""Construct a random Gaussian parameter initialiser, with mean 0
		standard deviation `1 / sqrt(input_dim)` where `input_dim` is the
...
class FanInInit(object):
	"""Initialisation to constrain estimated variance of a unit to be 
	independent of the number of incoming connections.
	Weights are sampled from a zero-mean uniform distribution with standard
	deviation `1 / sqrt(input_dim)` where `input_dim`
	is the input dimension of the weight matrix.
	"""
...
class FanOutInit(object):
	"""Initialisation to constrain estimated variance of a unit's gradient
	 to be independent of the number of outgoing connections.
	Weights are sampled from a zero-mean uniform distribution with standard
	deviation `1 / sqrt(ouput_dim)` where `output_dim`
	is the output dimension of the weight matrix.
	"""
